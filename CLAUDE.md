...
- `category`：分类，**必须对应** **`src/posts/`** **下的实际目录名**（如 `金融`、`大模型`、`刷题`、`架构设计`、`java基础` 等）
...
3. **金融类文章**：保存于 `src/posts/金融/`，须包含行业数据来源（天眼查、企查查、税局数据等）与可落地风控模型，结构遵循"背景→核心要素→应用场景→技术细节→案例→挑战"
